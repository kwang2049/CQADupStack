...
class Subforum():
    def __init__(self, zipped_catfile):
        ...
        self.__indri_stopwords = ['a', 'about', 'above', 'according', 'across', 'after', 'afterwards', 'again',
                                  'against', 'albeit', 'all', 'almost', 'alone', 'along', 'already', 'also', 'although',
                                  'always', 'am', 'among', 'amongst', 'an', 'and', 'another', 'any', 'anybody',
                                  'anyhow', 'anyone', 'anything', 'anyway', 'anywhere', 'apart', 'are', 'around', 'as',
                                  'at', 'av', 'be', 'became', 'because', 'become', 'becomes', 'becoming', 'been',
                                  'before', 'beforehand', 'behind', 'being', 'below', 'beside', 'besides', 'between',
                                  'beyond', 'both', 'but', 'by', 'can', 'cannot', 'canst', 'certain', 'cf', 'choose',
                                  ...
                                  'forth', 'forward', 'from', 'front', 'further', 'furthermore', 'furthest', 'get',
                                  'go', 'had', 'halves', 'hardly', 'has', 'hast', 'hath', 'have', 'he', 'hence',
                                  'henceforth', 'her', 'here', 'hereabouts', 'hereafter', 'hereby', 'herein', 'hereto',
                                  'hereupon', 'hers', 'herself', 'him', 'himself', 'hindmost', 'his', 'hither',
                                  'hitherto', 'how', 'however', 'howsoever', 'i', 'ie', 'if', 'in', 'inasmuch', 'inc',
                                  'include', 'included', 'including', 'indeed', 'indoors', 'inside', 'insomuch',
                                  'instead', 'into', 'inward', 'inwards', 'is', 'it', 'its', 'itself', 'just', 'kind',
                                  'kg', 'km', 'last', 'latter', 'latterly', 'less', 'lest', 'let', 'like', 'little',
                                  'ltd', 'many', 'may', 'maybe', 'me', 'meantime', 'meanwhile', 'might', 'moreover',
                                  'most', 'mostly', 'more', 'mr', 'mrs', 'ms', 'much', 'must', 'my', 'myself', 'namely',
                                  'need', 'neither', 'never', 'nevertheless', 'next', 'no', 'nobody', 'none',
                                  'nonetheless', 'noone', 'nope', 'nor', 'not', 'nothing', 'notwithstanding', 'now',
                                  'nowadays', 'nowhere', 'of', 'off', 'often', 'ok', 'on', 'once', 'one', 'only',
                                  'onto', 'or', 'other', 'others', 'otherwise', 'ought', 'our', 'ours', 'ourselves',
                                  'out', 'outside', 'over', 'own', 'per', 'perhaps', 'plenty', 'provide', 'quite',
                                  'rather', 'really', 'round', 'said', 'sake', 'same', 'sang', 'save', 'saw', 'see',
                                  ...
                                  'something', 'sometime', 'sometimes', 'somewhat', 'somewhere', 'spake', 'spat',
                                  'spoke', 'spoken', 'sprang', 'sprung', 'stave', 'staves', 'still', 'such',
                                  'supposing', 'than', 'that', 'the', 'thee', 'their', 'them', 'themselves', 'then',
                                  'thence', 'thenceforth', 'there', 'thereabout', 'thereabouts', 'thereafter',
                                  'thereby', 'therefore', 'therein', 'thereof', 'thereon', 'thereto', 'thereupon',
                                  'these', 'they', 'this', 'those', 'thou', 'though', 'thrice', 'through', 'throughout',
                                  'thru', 'thus', 'thy', 'thyself', 'till', 'to', 'together', 'too', 'toward',
                                  'towards', 'ugh', 'unable', 'under', 'underneath', 'unless', 'unlike', 'until', 'up',
                                  'upon', 'upward', 'upwards', 'us', 'use', 'used', 'using', 'very', 'via', 'vs',
                                  'want', 'was', 'we', 'week', 'well', 'were', 'what', 'whatever', 'whatsoever', 'when',
                                  'whence', 'whenever', 'whensoever', 'where', 'whereabouts', 'whereafter', 'whereas',
                                  'whereat', 'whereby', 'wherefore', 'wherefrom', 'wherein', 'whereinto', 'whereof',
                                  'whereon', 'wheresoever', 'whereto', 'whereunto', 'whereupon', 'wherever',
                                  'wherewith', 'whether', 'whew', 'which', 'whichever', 'whichsoever', 'while',
                                  'whilst', 'whither', 'who', 'whoa', 'whoever', 'whole', 'whom', 'whomever',
                                  'whomsoever', 'whose', 'whosoever', 'why', 'will', 'wilt', 'with', 'within',
                                  'without', 'worse', 'worst', 'would', 'wow', 'ye', 'yet', 'year', 'yippee', 'you',
                                  'your', 'yours', 'yourself', 'yourselves']
        self.__short_stopwords = ['a', 'an', 'the', 'yes', 'no', 'thanks']
        self.__middle_stopwords = ['in', 'on', 'at', 'a', 'an', 'is', 'be', 'was', 'I', 'you', 'the', 'do', 'did', 'of',
    ...
    def _unzip_and_load(self, zipped_catfile):
        ...
        if os.path.exists(questionfile) and os.path.exists(answerfile) and os.path.exists(userfile) and os.path.exists(
                commentfile):
    ...
    def get_ordered_list_of_posts(self):
        ...
        return sorted([(i, datetime.datetime.strptime(self.get_postdate(i), '%Y-%m-%d')) for i in self.postdict],
    ...
    def change_to_default_stopwords(self, stopwordset='middle'):
        ...
        if stopwordset == 'nltk':
            if self.__nltk_stopwords != []:
                ...
                self.__stopwords = [u'i', u'me', u'my', u'myself', u'we', u'our', u'ours', u'ourselves', u'you',
    ...
    def _general_cleaning(self, s, codes, remove_punct=False):
        ...
        for url in set(urls):
            if re.search(r'\)', url) and not re.search('\(', url):
                url = re.sub(r'\).*$', '', url)
            if re.search(r'\\',
                         url):  # Get rid of backslashes because else we get regex problems when trying to put the URLs back.
                url = re.sub(r'\\', '/', url)
        ...
        # restore URLs
        newurllist = itertools.izip(reversed(xrange(len(newurls))), reversed(
            newurls))  # reverse list to GGGG1 does not match GGG10. (Source: http://galvanist.com/post/53478841501/python-reverse-enumerate)
        ...
        # restore codeblocks
        newlist = itertools.izip(reversed(xrange(len(codes))), reversed(
            codes))  # reverse list to hhhh1 does not match hhhh10 (Source: http://galvanist.com/post/53478841501/python-reverse-enumerate)
        for i, c in newlist:
            s = re.sub('hhhh' + str(i), c.encode('unicode-escape'),
                       s)  # Escaping here leads to backslashes being added in the code blocks.
        return s
    ...
    def split_for_classification(self, outputdir='.'):
        ...
        nrofnondups_forsmallset = len(
            withdups) * 10  # Take ten times the nr of duplicate pairs as non-duplicate pairs. The rest of the non-duplicate pairs will not be used in the small test set.
        ...
        filenrs = [trainfile1, trainfile2, trainfile3, trainfile4, trainfile5, trainfile6, trainfile7, trainfile8,
                   trainfile9, trainfile10]
        dupindex = 0
        nondupindex = 0
        ...
        for i, ranking in enumerate(all_rankings):
            ...
            if relevantdocs != []:  # or (relateddocs != [] and include_related_posts): # We only compute the score for queries that actually do have duplicates in the indexed set.
                ...
                if include_related_posts:
                    relevance_judgements = map(
                        lambda x: 1.0 if x in relevantdocs else (0.5 if x in relateddocs else 0.0), ranking)
                else:
                ...
                av_nDCG = round(sum(nDCG) / float(len(nDCG)),
                                4)  # This is the same as round(sum(nDCG) * len(nDCG) ** -1, 4)
    ...
    def average_precision_at(self, scorefile, cutoff=None, include_related_posts=False):
        ...
        for i, ranking in enumerate(all_rankings):
            ...
            if relevantdocs != []:  # or (relateddocs != [] and include_related_posts): # We only compute the score for queries that actually do have duplicates in the indexed set.
                ...
                if cutoff:
                    ranking = ranking[:cutoff]
                relevant_in_ranking = len([w for w in relevantdocs if
                                           w in ranking])  # relevant docs is most likely shorter than ranking, so it is fastest this way around.
                if include_related_posts:
                    half_relevant_in_ranking = len([w for w in relateddocs if w in ranking]) / 2.0
                    relevant_in_ranking += half_relevant_in_ranking
    ...
    def _average_precision(self, ranking, relevantdocs, relateddocs=[]):
        ...
        for rank, doc in enumerate(ranking):
            if doc in relevantdocs:
                numerator += 1
                precisions.append(numerator / float(
                    rank + 1))  # enumerate starts to count from 0, but for the AP we need to start from 1. Hence the +1 here.
    ...
    def evaluate_classification(self, scorefile):
        ...
        prec_pos, prec_neg = self._compute_precision_oneclass(truenegatives, truepositives, falsenegatives,
                                                              falsepositives)
        ...
        return {'precision': precision, 'recall': recall, 'fscore': fscore, 'accuracy': accuracy,
                'precision_positive_class': prec_pos, 'precision_negative_class': prec_neg,
                'recall_positive_class': rec_pos, 'recall_negative_class': rec_neg}
    ...
    def plot_roc(self, scorefile, plotfilename):
        ...
        scores = np.array(
            scores_list)  # the scores can either be probability estimates of the positive class, confidence values, or binary decisions.
